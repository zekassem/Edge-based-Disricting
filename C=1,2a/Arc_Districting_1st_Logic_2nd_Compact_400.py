...
tol=[0.08,0.1]# tolerance parameter into the balancing constraints
...
def logic_cuts(c,w_v,edges,x_v,no_edges): # function that adds the logic cuts
    ...
    for i in node:
        ...
        coeff = []
        x_variables = []
# Logic Cut sum (i,k) e delta(i) xi,(i,k)-|delta(i)| * wi-|delta(i)| * zi >=-|delta(i)|  for i e V
    sum_x = []
    coeff = []
    x_variables = []
    for i in node:
...
for prob in probs:
    ...
    for num in no_dist:
        for t in tol:
            ...
            with open('Results/1st_Logic_2nd_Compact_no_dist_'+str(num)+'_tol_'+str(t)+'_prob_'+str(prob)+'.csv','w') as newFile:

...
tol=[0.20]# tolerance parameter into the balancing constraints
tol_2=[0.20] # tolerance for parity
...
def constraints_wo_cuts(c,edges_v, no_nodes,no_edges,no_dist,t,x_v,w_v,z_v_n,z_nau_v,r_v,t_2,node_part): #  This function adds all of the constraints for the original districting problem
    ...
    sum_x = []
    coeff = []
    # no_dist*rj>=sum i e V z_nau_ij for j e V^e
    for j in node_part["even"]:
        sum_x.append(r_v[j - 1])
    ...
    sum_x = []
    coeff = []
    # rj<=sum i e V z_nau_ij-1 for j e V^o
    ...
    sum_x = []
    coeff = []
    # no_dist * rj>=sum i e V z_nau_ij-1 for j e V^o
    for j in node_part["odd"]:
        sum_x.append(r_v[j - 1])
        coeff.append(no_dist)
        indicies = [(j - 1) + (q - 1) * no_nodes for q in node]
    ...
    for num in no_dist:
        for t in tol:
            for t_2 in tol_2:
                ...
                with open('Results/1st_Logic_no_dist_'+str(num)+'_tol_1_'+str(t)+'_tol_2_'+str(t_2)+'_prob_'+str(prob)+'.csv','w') as newFile:

...
Time_1st_Cut_wo_z=np.loadtxt(alg_file, skiprows=1, max_rows=281, usecols=2,delimiter=',',dtype=float).tolist()
Time_1st_Cut_w_z=np.loadtxt(alg_file, skiprows=1, max_rows=281, usecols=3,delimiter=',',dtype=float).tolist()
...
plt.savefig(r'fig/Performance_Profile.png')
